...
# Straight from GIT
#bash <( curl -qfsSL "https://raw.githubusercontent.com/pkgforge-security/api-keys/main/scripts/aki_bbot_secrets_yml_checker.sh") -a "/path/to/bbot/secrets.yml" --quota

#Help / Usage
if [[ "$*" == *"-h"* ]] || [[ "$*" == *"--help"* ]] || [[ "$*" == *"help"* ]] ; then
  echo -e "${YELLOW}➼ Usage${NC}: ${PURPLE}aki-bbot${NC} ${BLUE}-a${NC} ${GREEN}<your/bbot/secrets.yml>${NC}\n"
  echo -e "➼ ${BLUE}Extended Help${NC} :\n"
    if [ ! -f "$HOME/.config/bbot/secrets.yml" ]; then
        echo -e "Your ${YELLOW}$HOME/.config/bbot/secrets.yml${NC} ${RED}does not exist${NC}\nYou ${GREEN}must create${NC} one: ${BLUE}https://github.com/owasp-bbot/bbot/blob/master/examples/secrets.yml${NC}\nElse use:"
...
#Defaults
bbot_config_def="$HOME/.config/bbot/secrets.yml"

#if no input passed to -a or --bbot 
if [ -z "$bbot_config" ]; then
        # Check if default bbot config file exists
       if [[ -f $bbot_config_def ]]; then
            echo -e "${GREEN}ⓘ Using default ${BLUE}bbot config file${NC}: ${PURPLE}$bbot_config_def${NC}\n"
            export bbot_config=$bbot_config_def
       else
           echo -e "${RED}\u2717 Couldn't find${NC} the default${NC} ${PURPLE}bbot config file${NC}: ${RED}$bbot_config_def${NC}"
           echo -e "${YELLOW}specify it manually using${NC} ${BLUE}-a${NC} | ${BLUE}--bbot${NC}\n"
            ...
            if [[ -z $subfinder_config ]]; then
                ...
                if [[ -f $subfinder_config_def ]]; then
                   ...
                   export subfinder_config=$subfinder_config_def
                    ...
                    echo -e "${RED}\u2717 Fatal${NC}: Neither one of${PURPLE} bbot${NC} | ${PURPLE}subfinder${NC}  ${BLUE}options was used${NC} nor a ${PURPLE}default config file${NC} [${GREEN}bbot${NC}: ${PURPLE}$bbot_config_def${NC} ${RED}Not Found${NC}] [${GREEN}subfinder${NC}: ${PURPLE}$subfinder_config_def${NC} ${RED}Not Found${NC}]\n"
                exit 1
                fi
            fi
        fi  
else
    echo -e "${GREEN}ⓘ Using Specified ${BLUE}bbot config file${NC}: ${PURPLE}$bbot_config${NC}\n"  
         if [ ! -f  $bbot_config ]; then 
            echo -e "${RED}\u2717 Fatal${NC}: [ ${BLUE}bbot config file${NC}: ${PURPLE}$bbot_config${NC} ${RED}Not Found${NC} ]"
              if  [ -f  $bbot_config_def ]; then  
                 echo -e "${GREEN}ⓘ Using default ${BLUE}bbot config file${NC}: ${PURPLE}$bbot_config_def${NC}\n"            
                 export bbot_config=$bbot_config_def
              else
                 echo -e "${RED}\u2717 Fatal${NC}: [ ${BLUE}Default${NC}: ${PURPLE}$bbot_config_def${NC} ${RED}also Not Found${NC} ]\n"              
              fi        
         fi       
fi
...
#Re Check
#bbot
if [ -n "$bbot_config" ] && [ -e "$bbot_config" ]; then
  echo -e "${YELLOW}Check ${GREEN}bbot${NC} ${YELLOW}?${NC} : ${BLUE}Yes $(echo -e "${GREEN}\u2713${NC}")${NC}"
else
  echo -e "${YELLOW}Check ${GREEN}bbot${NC} ${YELLOW}?${NC} : ${RED}No $(echo -e "${RED}\u2717${NC}")${NC}"
fi
...
#bbot parser
if [ -n "$bbot_config" ]; then
    bbot_config_parsed=$(mktemp) && export bbot_config_parsed="$bbot_config_parsed"
    #Parse using yq (strip comments)
    cat "$bbot_config" | yq '... comments=""' > "$bbot_config_parsed"
#Banner
echo -e "${YELLOW}\n"
cat << "EOF"
.^~!7???7!^.                .^!7???7!~^.
...
echo -e "${NC}\n"
    #Main Functions
   #ASNLookup  
    ASNLookup_api_keys=$(yq eval '.modules.asnlookup.api_key' $bbot_config_parsed | grep -v 'null')
    ...
    #AlienVault
    alienvault_api_keys=$(yq eval '.modules.alienvault.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
    ...
    #BeVigil
    BeVigil_api_keys=$(yq eval '.modules.bevigil.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
    ...
    #BigDataCloud  
    BigDataCloud_api_keys=$(yq eval '.modules.bigdata.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #BinaryEdge  
    BinaryEdge_api_keys=$(yq eval '.modules.binaryedge.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #BuiltWith  
    BuiltWith_api_keys=$(yq eval '.modules.builtwith.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Censys  
    Censys_api_keys=$(yq eval '.modules.censys' $bbot_config_parsed -o json | jq -r '. | "\(.api_id):\(.api_secret)"' | grep -v 'null')
        invalid_key_found=false
   ...
   #CertCentral  
    #CertCentral_api_keys=$(yq eval '.modules.bevigil.api_key' $bbot_config_parsed -o json | jq -r '"\(.username):\(.apikey)"' | grep -v 'null')
    CertCentral_api_keys=$(yq eval '.modules.certcentral.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   ##certspotter  
    certspotter_api_keys=$(yq eval '.modules.certspotter.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Chaos  
    Chaos_api_keys=$(yq eval '.modules.chaos.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Cloudflare  
    Cloudflare_api_keys=$(yq eval '.modules.cloudflare.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #FullHunt  
    FullHunt_api_keys=$(yq eval '.modules.fullhunt.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #GitHub  
    GitHub_api_keys=$(yq eval '.modules.github.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #GitLab  
    GitLab_api_keys=$(yq eval '.modules.gitlab.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Hunter  
    Hunter_api_keys=$(yq eval '.modules.hunter.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #IntelX          
    IntelX_api_keys=$(yq eval '.modules.intelx.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #IPdata  
    IPdata_api_keys=$(yq eval '.modules.ipdata.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #IPinfo  
    IPinfo_api_keys=$(yq eval '.modules.ipinfo.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #ipstack  
    ipstack_api_keys=$(yq eval '.modules.ipstack.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #LeakIX  
    LeakIX_api_keys=$(yq eval '.modules.leakix.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Netlas  
    Netlas_api_keys=$(yq eval '.modules.netlas.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #NetworksDB  
    NetworksDB_api_keys=$(yq eval '.modules.networksdb.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #PassiveTotal  
    PassiveTotal_api_keys=$(yq eval '.modules.passivetotal' $bbot_config_parsed -o json | jq -r '. | "\(.username):\(.api_key)"' | grep -v 'null')
        invalid_key_found=false
   ...
   #Pastebin  
    Pastebin_api_keys=$(yq eval '.modules.pastebin.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #PublicWWW  
    PublicWWW_api_keys=$(yq eval '.modules.publicwww.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Shodan  
    Shodan_api_keys=$(yq eval '.modules.shodan_dns.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #SecurityTrails  
    SecurityTrails_api_keys=$(yq eval '.modules.securitytrails.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #Spamhaus  
     Spamhaus_api_keys=$(yq eval '.modules.spamhaus.api_key' $bbot_config_parsed | grep -v 'null')
     invalid_key_found=false
   ...
   #URLScan  
    URLScan_api_keys=$(yq eval '.modules.urlscan.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #VirusTotal  
    VirusTotal_api_keys=$(yq eval '.modules.virustotal.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
   ...
   #WhoisXML  
    WhoisXML_api_keys=$(yq eval '.modules.whoisxmlapi.api_key' $bbot_config_parsed | grep -v 'null')
    invalid_key_found=false
...
# yq eval '.modules.bevigil.api_key' $bbot_config_parsed | grep -v 'null'        
   #ZoomEyeAPI  
     ZoomEye_api_keys=$(yq eval '.modules.zoomeye.api_key' $bbot_config_parsed | grep -v 'null')
     invalid_key_found=false
